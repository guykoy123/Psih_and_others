...
public class Test_Dummy_Controller : MonoBehaviour {
	...
	void Start ()
    {
        ...
        Enemy.Set_Heal_Delay(0.8f); //set low heal delay because dummy can not die
	}
	...
	void Update ()
    {
        if (Enemy.Get_Current_Health() < Enemy.Get_Base_Health()) //check if recieved damage
            Debug.Log(Enemy.Heal(Enemy.Get_Base_Health() - Enemy.Get_Current_Health())); //heal to full health
    }
}
